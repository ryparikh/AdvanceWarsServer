...
| `-test-evaluate` | Run focused checkpoint evaluation tests. | Generates tiny evaluation reports, checks side swapping, output overwrite protection, and checkpoint agent validation. |
...
| `-evaluate --agent0 <checkpoint-policy\|random> --agent1 <checkpoint-policy\|random> --map <id> --player0-co <id> --player1-co <id> --rounds <n> --out <path> [options]` | Evaluate checkpoint agents and write a report. | V1 is report-only: it writes an `evaluation.json` style artifact, keeps true draws separate from `action-limit` no-results, and never mutates checkpoint bundles. See `docs/EVALUATION_COMMAND_DESIGN.md`. |
